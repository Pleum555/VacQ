...
exports.getAppointment=async (req,res,next)=>{
    try{
        ...
        if(!appointment){
            return res.status(404).json({success:false, message:`No appointment with the id if ${req.params.id}`});
        }
...
exports.addAppointment=async (req,res,next)=>{
    try{
        ...
        if(existedAppointments.length >= 3 && req.body.role !== 'admin'){
...
exports.updateAppointment=async (req,res,next)=>{
    try{
        ...
        if(appointment.user.toString()!== req.user.id && req.user.role!== 'admin'){
            res.status(401).json({success:false, message:`User ${req.user.id} is not authorized to update this appointment`});
        }
...
exports.deleteAppointment=async (req,res,next)=>{
    try{
        ...
        //Make sure user is the appointment owner
        if(appointment.user.toString()!== req.user.id && req.user.role!== 'admin'){
            res.status(401).json({success:false, message:`User ${req.user.id} is not authorized to delete this appointment`});
        }
